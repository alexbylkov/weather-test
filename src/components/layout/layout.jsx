...
import { dateFormatterLong, getWeatherStrStrong, getTemp } from '../../helpers'
import { GoogleSelect } from '../../containers/google-select'
import { Link } from 'react-router-dom'

import './layout.sass'

export const Layout = ({ children, setTab, weatherCurrent, cityName, saveCity }) => {

  const tabName = ['today', 'tomorrow', 'week']
  ...
  return (
    <>
      ...
      <main>
        <div className="info">
          <span className="info__temp">{getTemp(weatherCurrent.temp)}</span>
          <h2 className="info__city">{cityName}</h2>
          <h2 className="info__date">{dateFormatterLong(weatherCurrent.dt)}</h2>
          <span className="info__weather">{getWeatherStrStrong(weatherCurrent)}</span>
          <button 
            className="info__add"
            onClick={saveCity}
          >+</button>
          <Link
            className="info__home"
            to={'/'}
          >
